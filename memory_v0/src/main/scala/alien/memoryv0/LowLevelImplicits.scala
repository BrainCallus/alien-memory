package alien.memoryv0

import alien.memoryv0.layout.{
  :=,
  <>:,
  >>:,
  Aligned,
...
import alien.memoryv0.typelevel.pow2.*
import alien.memoryv0.layout.{DynamicHead, DynamicStruct, DynamicUnion}
...
trait LowLevelImplicits {
  ...
  implicit def recurseSUnwrap[L <: DynamicStruct, L1, Head <: layout.:=[?, ?]](
    implicit
    unwrap: Unwrap[L, L1],
  ...
  implicit def recurseUUnwrap[L <: DynamicUnion, L1, Head <: layout.:=[?, ?]](
    implicit
    unwrap: Unwrap[L, L1],
  ): Unwrap[layout.<>:[Head, L], L1] = s => unwrap(s.tail)
